...
**A novel OCR system using a custom-designed Hindi-like alphabet optimized for mechanical distinguishability and handwritten legibility.** This project introduces a refined set of 60 symbols tailored to simplify handwriting and enhance OCR accuracy, along with an end-to-end pipeline for training and inference using K-Nearest Neighbors (KNN).
...
``
3. From ``submit`` run ``knn.py`` to create the ``model.joblib`` file.
4. Add your test image to this directory and paste it's path in the image_path from ``ocr.py`` and run it to get the results. 
...
hindi_ocr_revised/
├── TOOLS/
│   ├── augmentation.py          # Script for data augmentation (rotations, noise, etc.)
│   ├── data_cleaning.py         # Preprocessing and cleaning raw image data
│   └── get_data_segmentation.py # Segmenting characters from raw samples
├── characters/                  # Final set of designed symbol images
├── sample images/               # Raw handwritten character images (30×30 px)
├── sample images copy/          # Augmented dataset derived from raw samples
├── submit/
│   ├── knn.py                   # Training script: generates model.joblib
│   ├── pre.py                   # Preprocessing for OCR inference
│   └── ocr.py                   # Performs OCR using trained model
├── theory/                      # Documentation & concept PDF
│   └── Hindi_Script_Refinement___Improved_OCR.pdf
├── knn2.py                      # Alternate or extended KNN implementation
...
- **scikit-learn** – KNN classifier and model training
- **joblib** – saving/loading trained models
- **opencv-python** – image processing (reading, resizing, augmentations)
- **matplotlib** – visualization of characters and results
- **scipy** – additional scientific computations
- **Pillow (PIL)** – image manipulation
- **tqdm** – progress bars for dataset preprocessing

For installation, you can also add:

```
pip install numpy pandas scikit-learn joblib opencv-python matplotlib scipy pillow tqdm
```
...
WE HAVE DESIGNED SOME (64) CHARACTERS AS ALPHABETS (Check out the [sample alphabet design](https://github.com/amansingh2116/hindi_ocr_revised/blob/main/characters/alpha1.jpeg).
) AS FIRST PROTOTYPE BEFORE ARRIVING AT THE FINAL SET OF ALPHABETS, AND PREPARED A TRAINING DATA SET FOR OCR HAVING AROUND 120 HANDWRITTEN VARIATIONS OF EACH CHARACTERS.
...
- **Classification:** Once the model is trained, we use it to classify the handwritten symbols by predicting the corresponding alphabet symbols. To ensure accurate classification, we had to digitally create our new symbols/characters so we could match them with our data. Here are the [final characters](https://github.com/amansingh2116/hindi_ocr_revised/blob/main/characters/final_char.jpeg).
